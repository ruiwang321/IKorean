 ...
 *
 */
static AdViewController *gAdViewControllers;
...
@synthesize adViewsDict = adViewsDict_;
 ...
 */
+ (AdViewController*) sharedControllerByAppKey
{
    if (nil == gAdViewControllers)
    {
        @synchronized(gLockObj) {
            if (nil == gAdViewControllers) {
                gAdViewControllers = [[AdViewController alloc] initWithNibName:nil bundle:[NSBundle mainBundle]];
                gAdViewControllers.adViewsDict = [NSMutableDictionary dictionary];
                
            }
        }
    }
    return gAdViewControllers;
}
/**
 *  <#Description#>
 *
 ...
 *  @return <#return value description#>
 */
+ (AdViewController*) sharedControllerIfExistsByAppKey:(NSString*)appKey
{
    if (nil == appKey) return nil;
    for (NSString* str in [gAdViewControllers.adViewsDict allKeys]) {
        if ([str isEqualToString:appKey]) {
            gAdViewControllers.adView = [gAdViewControllers.adViewsDict objectForKey:appKey];
            return gAdViewControllers;
        }
    }
    gAdViewControllers.adView = nil;
    return gAdViewControllers;
}
/**
 *  <#Description#>
 ...
 *  @param appKey <#appKey description#>
 */
+ (void) deleteControllerByAppKey:(NSString*)appKey
{
    if (nil == appKey) return;
    
    if (nil == gAdViewControllers) return;
    ...
    @synchronized(gLockObj) {
        ...
        for (NSString *str in [gAdViewControllers.adViewsDict allKeys]) {
            if ([str isEqualToString:appKey]) {
                AdViewView *viewAd = (AdViewView*)[gAdVAdController.adViewsDict objectForKey:appKey];
                [viewAd removeFromSuperview];
                viewAd = nil;
                [gAdVAdController.adViewsDict removeObjectForKey:appKey];
            }
        }
    }
}
...
- (void)addAdView {
     ...
     *
     */
    if (nil != self.adView && nil == gAdViewControllers) {
     ...
     *  <#Description#>
     */
    if (gAdViewControllers && nil != self.adView) {
        for (NSString*str in [gAdViewControllers.adViewsDict allKeys]) {
            if ([str isEqualToString:adviewKey]) {
                [gAdViewControllers.adViewsDict removeObjectForKey:adviewKey];
            }
        }
        [gAdViewControllers.adViewsDict setObject:self.adView forKey:adviewKey];
    }
